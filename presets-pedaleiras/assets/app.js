...
const products=Object.entries(catalog).flatMap(([brand,models])=>models.map(model=>{
 ...
 return {id:nextId++,brand,slug:slugify(brand),model:model.toUpperCase(),name:`${model} Tone Pack`,meta:featured?'Presets + Capturas + IRs':'Presets profissionais + bônus',price:67,checkoutUrl:'#',badge:featured?'MAIS VENDIDO':(model==='Matribox 2 Pro'?'TONE LAB':undefined)};
}));
...
function render(){const q=search.value.toLowerCase().trim();const list=products.filter(p=>(activeFilter==='all'||p.slug===activeFilter)&&(`${p.brand} ${p.model} ${p.name} ${p.meta}`.toLowerCase().includes(q)));grid.innerHTML=list.map(p=>`<article class="product" data-brand="${p.slug}">${p.badge?`<span class="badge">${p.badge}</span>`:''}<div class="product-art"><b>${p.model.replace('\n','<br>')}<small>${p.brand}</small></b></div><div class="product-info"><span class="product-brand">${p.brand} // TONE PACK</span><h3>${p.name}</h3><span class="product-meta">${p.meta}</span><div class="product-buy"><div class="price"><small>POR APENAS</small><strong>${money(p.price)}</strong></div><a class="btn primary product-checkout" href="${p.checkoutUrl}" aria-label="Comprar ${p.name}">COMPRAR <span>↗</span></a></div></div></article>`).join('');empty.hidden=list.length>0;}
...
document.querySelector('.menu-toggle').onclick=e=>{const nav=document.querySelector('.nav');nav.classList.toggle('open');e.currentTarget.setAttribute('aria-expanded',nav.classList.contains('open'))};render();
